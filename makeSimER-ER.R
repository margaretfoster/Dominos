...
bN$nodeInformation[which(bN$nodeInformation$egoW==1),"egoW"] <- .9
bN$nodeInformation[which(bN$nodeInformation$egoW==0),"egoW"] <- .1

## Go in and print a flag if any node is isolated
...
ten.Rounds <- callLoop(bothNets=bN,
                       round=10,
                       rshock= 6,
                       pts=.3,## prop To Shock
                       rseed=71621)
